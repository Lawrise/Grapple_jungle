...
class Level:
	...
	def setup_level(self,layout):
		...
		for row_index,row in enumerate(layout):
			for col_index,cell in enumerate(row):
				...
				if cell == 'C':
					caisse = Caisse((x,y),self.display_surface)
					self.caisses.add(caisse)

	def scroll__camera(self):
	...
	def horizontal_movement_collision(self):
		...
		for caisse in self.caisses.sprites():
			if caisse.rect.colliderect(player.rect):
				if player.direction.x < 0:
					player.rect.left = sprite.rect.right
					player.on_left = True
					self.current_x = player.rect.left
	...
	def run(self):
		...
		# Caisse
		self.caisses.draw(self.display_surface)
		self.caisses.update(self.world_shift, self.world_shift_y)
